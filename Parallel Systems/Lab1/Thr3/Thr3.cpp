...
unsigned thridR;		//thread identifier
unsigned thridX;		//thread identifier
...
void TerminateX(HWND hWnd, HMENU hMenu, BOOL * fTerminateX);
...
//	int wmId, wmEvent;
   ...
   return 0;//BY CHANCE TO BE USEDthridE
}
LONG WndProc_OnCommand (HWND hWnd,int id,HWND hwndCtl,UINT codeNotify)
{
		// Parse the menu selections:
			switch (id)
			{
				...
				case IDM_SUSX:
					SuspendX(GetMenu(hWnd), &bSuspendX);
					return 0L;
				...
				case IDM_TERMX:
					TerminateX(hWnd, GetMenu(hWnd), &fTerminateX);
					return 0L;
...
LONG WndProc_OnDestroy(HWND hWnd)
{
	...
	std::vector<HANDLE> notTermTh;
	if (!fTerminateE)
		notTermTh.push_back(hThreadE[0]);
	if (!fTerminateR)
		notTermTh.push_back(hThreadE[1]);
	if (!fTerminateX)
		notTermTh.push_back(hThreadE[2]);
	int numOfThreads = notTermTh.size();
	HANDLE* notTermArr = new HANDLE[numOfThreads];
	std::copy(notTermTh.begin(), notTermTh.end(), notTermArr);
	...
	switch(dwRet=WaitForMultipleObjects(numOfThreads,notTermArr,TRUE,INFINITE))
	{
	case WAIT_ABANDONED_0 :	
		...
		break;
	case WAIT_FAILED:
		wsprintf(szRetRes, TEXT("WAIT_FAILED getLastError=%d"), GetLastError());
		MessageBox(NULL, szRetRes, "WaitForMultipleObjects", MB_OK | MB_ICONEXCLAMATION);
		break;
	case WAIT_OBJECT_0:	
	...
	for (int i = 0; i < numOfThreads; i++) {
		if (!CloseHandle(notTermArr[i])) {
			MessageBox(NULL, "CloseHandle  failed",
				"One of Threads", MB_OK | MB_ICONEXCLAMATION);
		}
	}
	...
	{
		MessageBox(NULL, "CloseHandle  failed",
							"PaintX Thread", MB_OK | MB_ICONEXCLAMATION);
	};*/
...
unsigned int __stdcall  PaintX(void* hWnd) {
	...
	while (!fTerminateX) {
		...
		hBrush = CreateSolidBrush(RGB(nRed, nGreen, nBlue));
		hOldBrush = (HBRUSH)SelectObject(hDC, hBrush);
		TextOut(hDC, min(xLeft, xRight), min(yTop, yBottom), "This is the string of the 3rd thread", 36);
		SelectObject(hDC, hOldBrush);
		DeleteObject(hBrush);
		ReleaseDC((HWND)hWnd, hDC);
		//Sleep(10000);
...
void SuspendX(HMENU hMenu, bool* bSuspend)
{
	...
	DWORD dwRetCode;
		if (!*bSuspend) {
			...
			{
				wsprintf(message, TEXT("SuspendThread Error %ld"), GetLastError());
				MessageBox(NULL, message, "PaintX Thread", MB_OK | MB_ICONEXCLAMATION);
				LeaveCriticalSection(&crSec);
				return;
			}
			...
			*bSuspend = true;
			//Check
			//Sets the check-mark attribute to the checked state.
			CheckMenuItem(hMenu, IDM_SUSX, MF_CHECKED);
			LeaveCriticalSection(&crSec);
		}
		else
		{
			...
			*bSuspend = false;
			//Uncheck 
			//Sets the check-mark attribute to the unchecked state.
			CheckMenuItem(hMenu, IDM_SUSX, MF_UNCHECKED);
		}
		return;
}//SuspendX
...
void TerminateX(HWND hWnd, HMENU hMenu, BOOL* fTerminateX)
{
	if (!*fTerminateX)
	{
		//Check
		//Sets the check-mark attribute to the unchecked state.
		CheckMenuItem(hMenu, IDM_TERMX, MF_CHECKED);
		*fTerminateX = true;
		if (bSuspendR)
			ResumeThread(hThreadE[2]);
	...
	else
	{
		//Uncheck
		//Sets the check-mark attribute to the checked state.
		CheckMenuItem(hMenu, IDM_TERMX, MF_UNCHECKED);

		*fTerminateX = false;

		unsigned ususpend = (bSuspendX) ? CREATE_SUSPENDED : 0;
