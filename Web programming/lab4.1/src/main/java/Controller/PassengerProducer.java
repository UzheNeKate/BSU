...
public class PassengerProducer implements Runnable {
     ...
     * @param maxInterval max interval to generate next passenger
     */
    public PassengerProducer(List<Route> routes, long maxInterval) {
        this.routes = routes;
    ...
    public void run() {
        while (true) {
            ...
            synchronized (stops.get(firstStop).getPassengerQueues()) {
                stops.get(firstStop).pushToQueue(passenger);
            }
            logger.info("New passenger added " + passenger);
            try {
                synchronized (stops.get(firstStop).getQueue(route)) {
                    stops.get(firstStop).getQueue(route).wait(Math.abs(random.nextLong()) % maxInterval);
                }
            } catch (InterruptedException e) {
                logger.warn("Something wrong in sleeping.");
                e.printStackTrace();
            }
        }
    }
